        ...
        if($request->ajax()){
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $actionBtn = '<a href="' . route('role.show', $row->id) . '" class="edit btn btn-success btn-sm">Show</a> &nbsp;&nbsp;';
                    if(auth()->user()->hasPermissionTo('role-edit'))
                    $actionBtn .= '<a href="' . route('role.edit', $row->id) . '" class="delete btn btn-primary btn-sm">Edit</a>';
                    if(auth()->user()->hasPermissionTo('role-delete'))
                    $actionBtn .= '<form method="POST" action="' . route('role.destroy', $row->id) . '" style="display: inline;">' . csrf_field() . ' <button type="submit" class="btn btn-danger btn-sm">Delete</button>' . method_field('DELETE') . ' </form>';
